...
class ControllerPaymentPayanywayWalletone extends Controller {
	...
	public function index() { 
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
			...
			$this->model_setting_setting->editSetting('payanyway_walletone', $this->request->post['payanyway_walletone']);
 		...
 		if (isset($this->error['warning'])) {
  		...
  		$this->data['breadcrumbs'] = array();
   		...
   		$this->data['breadcrumbs'][] = array(
			...
			'href'      => $this->url->link('payment/payanyway_walletone', 'token=' . $this->session->data['token'], 'SSL'),
      		'separator' => ' :: '
   		);
		
		$this->data['action'] = $this->url->link('payment/payanyway_walletone', 'token=' . $this->session->data['token'], 'SSL');
		...
		if (isset($this->request->post['payanyway_walletone_status'])) {
			$this->data['payanyway_walletone_status'] = $this->request->post['payanyway_walletone_status'];
		} else {
			$this->data['payanyway_walletone_status'] = $this->config->get('payanyway_walletone_status');
		}

		if (isset($this->request->post['payanyway_walletone_sort_order'])) {
			$this->data['payanyway_walletone_sort_order'] = $this->request->post['payanyway_walletone_sort_order'];
		} else {
			$this->data['payanyway_walletone_sort_order'] = $this->config->get('payanyway_walletone_sort_order');
		}

		$this->template = 'payment/payanyway_walletone.tpl';
		$this->children = array(
			'common/header',
			'common/footer',
		);
	...
	private function validate() {
		if (!$this->user->hasPermission('modify', 'payment/payanyway_walletone')) {
