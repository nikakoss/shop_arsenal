...
class ControllerPaymentPayanyway extends Controller {
	...
	public function index() {
		...
		$this->data['action'] = $this->url->link('payment/payanyway', 'token=' . $this->session->data['token'], 'SSL');
		...
		if (isset($this->request->post['payanyway_status'])) {
			$this->data['payanyway_status'] = $this->request->post['payanyway_status'];
		} else {
			$this->data['payanyway_status'] = $this->config->get('payanyway_status');
		}

		if (isset($this->request->post['payanyway_sort_order'])) {
			$this->data['payanyway_sort_order'] = $this->request->post['payanyway_sort_order'];
		} else {
			$this->data['payanyway_sort_order'] = $this->config->get('payanyway_sort_order');
		}

		if (isset($this->request->post['payanyway_username'])) {
			$this->data['payanyway_username'] = $this->request->post['payanyway_username'];
		} else {
			$this->data['payanyway_username'] = $this->config->get('payanyway_username');
		}

		if (isset($this->request->post['payanyway_password'])) {
			$this->data['payanyway_password'] = $this->request->post['payanyway_password'];
		} else {
			$this->data['payanyway_password'] = $this->config->get('payanyway_password');
		}

		$this->template = 'payment/payanyway.tpl';
		$this->children = array(
			'common/header',
			'common/footer',
		);
