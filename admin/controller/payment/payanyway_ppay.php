...
class ControllerPaymentPayanywayPPay extends Controller {
	...
	public function index() { 
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
			...
			$this->model_setting_setting->editSetting('payanyway_ppay', $this->request->post['payanyway_ppay']);
 		...
 		if (isset($this->error['warning'])) {
  		...
  		$this->data['breadcrumbs'] = array();
   		...
   		$this->data['breadcrumbs'][] = array(
			...
			'href'      => $this->url->link('payment/payanyway_ppay', 'token=' . $this->session->data['token'], 'SSL'),
      		'separator' => ' :: '
   		);
		
		$this->data['action'] = $this->url->link('payment/payanyway_ppay', 'token=' . $this->session->data['token'], 'SSL');
		...
		if (isset($this->request->post['payanyway_ppay_status'])) {
			$this->data['payanyway_ppay_status'] = $this->request->post['payanyway_ppay_status'];
		} else {
			$this->data['payanyway_ppay_status'] = $this->config->get('payanyway_ppay_status');
		}

		if (isset($this->request->post['payanyway_ppay_sort_order'])) {
			$this->data['payanyway_ppay_sort_order'] = $this->request->post['payanyway_ppay_sort_order'];
		} else {
			$this->data['payanyway_ppay_sort_order'] = $this->config->get('payanyway_ppay_sort_order');
		}

		$this->template = 'payment/payanyway_ppay.tpl';
		$this->children = array(
			'common/header',
			'common/footer',
		);
	...
	private function validate() {
		if (!$this->user->hasPermission('modify', 'payment/payanyway_ppay')) {
