...
class ControllerPaymentPayanywayMM extends Controller {
	...
	public function index() { 
		...
		$this->data['action'] = $this->url->link('payment/payanyway_mm', 'token=' . $this->session->data['token'], 'SSL');
		...
		if (isset($this->request->post['payanyway_mm_status'])) {
			$this->data['payanyway_mm_status'] = $this->request->post['payanyway_mm_status'];
		} else {
			$this->data['payanyway_mm_status'] = $this->config->get('payanyway_mm_status');
		}

		if (isset($this->request->post['payanyway_mm_sort_order'])) {
			$this->data['payanyway_mm_sort_order'] = $this->request->post['payanyway_mm_sort_order'];
		} else {
			$this->data['payanyway_mm_sort_order'] = $this->config->get('payanyway_mm_sort_order');
		}

		$this->template = 'payment/payanyway_mm.tpl';
		$this->children = array(
			'common/header',
			'common/footer',
		);
