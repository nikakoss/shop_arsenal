...
class ControllerSaleCoupon extends Controller {
    ...
    protected function getList() {
        ...
        $this->data['updateDiscount'] = $this->url->link('sale/coupon/updateDiscount', 'token=' . $this->session->data['token'] . $url, 'SSL');
        $this->data['coupons'] = array();
        
        /*-----------------------------promocode---------------------------------------*/
        $this->data['insertPromo'] = $this->url->link('sale/coupon/insertPromoCode', 'token=' . $this->session->data['token'] . $url, 'SSL');
        $this->data['deletePromo'] = $this->url->link('sale/coupon/deletePromo', 'token=' . $this->session->data['token'] . $url, 'SSL');
        ...
        foreach ($results as $result) {
            ...
            $this->data['coupons'][] = array(
                ...
                'name' => $result['name'],
                'code' => $result['code'],
                'status_client' => $result['client_name'],
        ...
        $results = $this->model_sale_coupon->getDiscountsData();
        
        foreach ($results->rows as  $result) {
            $this->data['discountData'][] = array(
                'name' => $result['name'],
                'discount' => $result['discount'],
                'sum' => $result['sum'],
                'url' => $result['url'],
        ...
        $this->data['promoList'] = array(); 
        
        $this->data['promoList'] = $this->model_sale_coupon->getPromoList();
    ...
    protected function getForm() {
        ...
        if (isset($this->request->post['status_client'])) {
            $this->data['type'] = $this->request->post['status_client'];
        } elseif (!empty($coupon_info)) {
            $this->data['status_client'] = $coupon_info['status_client'];
        } else {
            $this->data['status_client'] = '';
        }
    ...
    public function updateDiscount() {
        ...
        $error = 4;
        $this->data['clientStatus'] = array();         
        for ($i = 0; $i <= 3; $i++) {
            if (isset($this->request->post['client' . $i . 'discount']) && isset($this->request->post['client' . $i . 'sum']) && isset($this->request->post['client' . $i . 'name']) && isset($this->request->post['client' . $i . 'url'])) {

                if ($this->request->post['client' . $i . 'discount'] == '') {
                    $this->data['clientStatus'][$i] = array(
                        'discount' . $i => 0,
                        'sum' . $i => $this->request->post['client' . $i . 'sum'],
                        'name' . $i => $this->request->post['client' . $i . 'name'],
                        'url' . $i => $this->request->post['client' . $i . 'url'],
                    );
                } else if ($this->request->post['client' . $i . 'sum'] == '') {
                    $this->data['clientStatus'][$i] = array(
                        'sum' . $i => 0,
                        'discount' . $i => $this->request->post['client' . $i . 'discount'],
                        'name' . $i => $this->request->post['client' . $i . 'name'],
                        'name' . $i => $this->request->post['client' . $i . 'name'],
                        'url' . $i => $this->request->post['client' . $i . 'url'],
                    );
                } else if ($this->request->post['client' . $i . 'name'] == '') {
                    $this->data['clientStatus'][$i] = array(
                        'name' . $i => 'название',
                        'discount' . $i => $this->request->post['client' . $i . 'discount'],
                        'sum' . $i => $this->request->post['client' . $i . 'sum'],
                 ...
                 } else if ($this->request->post['client' . $i . 'url'] == '') {
                    $this->data['clientStatus'][$i] = array(
                        'url' . $i => '',
                        'name' . $i => $this->request->post['client' . $i . 'name'],
                        'discount' . $i => $this->request->post['client' . $i . 'discount'],
            ...
            if (!is_numeric($this->request->post['client' . $i . 'discount']) || !is_numeric($this->request->post['client' . $i . 'sum'])) {
                $error = 1;
            }
        ...
        if ($error == 0) {
            ...
            $this->model_sale_coupon->updateDiscount($this->data['clientStatus']);
        }       
        
        $this->getList();
    }
    //----------------------------promocode-------------------------------------//
    protected function validatePromoForm() {
        if (!$this->user->hasPermission('modify', 'sale/coupon')) {
            $this->error['warning'] = $this->language->get('error_permission');
        }

        if ((utf8_strlen($this->request->post['code']) < 3) || (utf8_strlen($this->request->post['code']) > 10)) {
            $this->error['code'] = "Пожалуйста введите обязательное поле.";
        }        
   ...
   public function deletePromo() {
        ...
        if (isset($this->request->post['promo_id'])) {
            
            $this->model_sale_coupon->deletePromo($this->request->post['promo_id']);
    ...
    public function insertPromoCode() {
        ...
        if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validatePromoForm()) {
            $this->model_sale_coupon->addPromo($this->request->post);
    ...
    protected function getPromoForm() {
        $this->data['heading_title'] = "Добавление промокода";

        $this->data['entry_code'] = "Код <span class='help'>Клиент вводит код для получения статуса</span>";
        $this->data['entry_type'] = "Тип покупателя <span class='help'>статуса покупателя будет повышен до:</span>";
        $this->data['entry_uses_total'] = "Количество использований промокода ";;
        ...
        $this->data['action'] = $this->url->link('sale/coupon/insertPromoCode', 'token=' . $this->session->data['token'] . $url, 'SSL');
       ...
       // }
        ...
        $this->data['status_names'] = array();
        $this->data['status_names'] = $this->model_sale_coupon->getStatusName();
        ...
        $this->template = 'sale/coupon_promo_form.tpl';
        $this->children = array(
            'common/header',
            'common/footer'
        );
