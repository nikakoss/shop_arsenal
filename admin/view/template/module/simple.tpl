...
function add_field(form_selector, msc_selector, fields_name, field_prefix) {
    ...
    $(msc_selector).append('<table id="'+id+'" class="form" style="margin-bottom:50px;">' +
          ...
          '<tr class="row_date" '+((type != 'date') ? 'style="display:none;"' : '')+'>' +
            ...
            '</td>' +
            '<td id="date">' +
                '<?php echo $entry_min_date ?>: <input name="'+fields_name+'['+id+'][date_min]" type="text" class="datepicker" value="'+date_min+'" > <?php echo $entry_start_date ?>: <input name="'+fields_name+'['+id+'][date_start]" type="text" size="1" value="'+date_start+'" ><br>' +
                '<?php echo $entry_max_date ?>: <input name="'+fields_name+'['+id+'][date_max]" type="text" class="datepicker" value="'+date_max+'" > <?php echo $entry_end_date ?>: <input name="'+fields_name+'['+id+'][date_end]" type="text" size="1" value="'+date_end+'" ><br>' +
                '<?php echo $entry_only_business_days ?>: <input name="'+fields_name+'['+id+'][date_only_business]" type="checkbox" value="1" ' + (date_only_business ? 'checked="checked"' : '') + '>' +
                '<?php echo $entry_only_for_days ?>: <input name="'+fields_name+'['+id+'][date_only_for]" type="input" value="' + date_only_for + '">' +
            '</td>  ' +
          '</tr>' +
