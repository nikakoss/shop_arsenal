...
class ModelSaleCoupon extends Model {
    ...
    public function getCoupons($data = array()) {
        $sql = "SELECT c.coupon_id,c.status_client,cn.name as client_name, c.name, c.code, c.discount, c.date_start, c.date_end, c.status FROM oc_coupon c, oc_coupon_name cn WHERE c.status_client=cn.id";
    ...
    public function updateDiscount($data) {
        ...
        for ($i = 0; count($data) > $i; $i++) {
            $this->db->query("UPDATE " . DB_PREFIX . "coupon_name SET name = '" . $data[$i]['name' . $i] . "', url = '" . $data[$i]['url' . $i] . "', discount = '" . $data[$i]['discount' . $i] . "', sum = '" . $data[$i]['sum' . $i] . "' WHERE id = $z");
            $z++;
        }
        $query = $this->db->query("SELECT  * FROM " . DB_PREFIX . "coupon_name");
        return $query;
    }
    ...
    public function getStatusName() {
        $query = $this->db->query("SELECT id, name FROM " . DB_PREFIX . "coupon_name");
        return $query->rows;
    }
    public function addPromo($data) {
        $this->db->query("INSERT INTO " . DB_PREFIX . "coupon_promo SET code = '" . $this->db->escape($data['code']) . "', uses = '" . (int) $this->db->escape($data['uses_total']) . "', 	status_id = '" . $this->db->escape($data['type'])."'");
    }
    public function getPromoList() {
        $query = $this->db->query("SELECT t1.*, t2.name FROM " . DB_PREFIX . "coupon_promo AS t1, " . DB_PREFIX . "coupon_name AS t2 WHERE t1.status_id = t2.id ORDER BY status_id");
        return $query->rows;        
    }
     public function deletePromo($id) {
        $this->db->query("DELETE FROM " . DB_PREFIX . "coupon_promo WHERE id = '" . (int) $id . "'");
    }
