    ...
    <div class="simplecheckout" id="simplecheckout_form">
        ...
        <script>
            function hideBlock(blockId) {
                $("#" + blockId).toggle();
                $("#" + blockId).prev('div').children('a').toggleClass('changeBG')
            }
            $("#pecom-kabinet-iframe").ready(function() {
                hideBlock('pecom-kabinet-iframe');
            });
        </script>
        ...
        <div class="simplecheckout-button-block buttons" id="buttons" <?php if ($block_order) { ?>style="display:none;"<?php } ?>>
            ...
            <?php } ?>
            <a href="javascript:void(0);" class="calckG" onclick="hideBlock('pecom-kabinet-iframe')">КАЛЬКУЛЯТОР ГРУЗОПЕРЕВОЗКИ</a>
        </div>
       
        <iframe id="pecom-kabinet-iframe" allowtransparency="true" frameborder="0" width="920" height="1230" scrolling="auto" style="border: 0;"
                src="http://www.pecom.ru/ru/calc/?iframe=Y">
        <p>Пожалуйста, воспользуйтесь браузером, который поддерживает IFrame'ы.</p>
        </iframe>
    </div>
    <?php if (!$ajax) { ?>
</div>
...
<?php } ?>
<?php
if ($this->customer->isLogged() && $_SESSION['shipping_method']['code'] == 'pickup.pickup'){ ?>
<script>
$('#simplecheckout_customer').css('display','none');
