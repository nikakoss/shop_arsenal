<?php if($categories){ ?> 
<div class="dashboard-nav popravka">
    <ul>
        <li class="kat_act"><a id="menu_top">Категории товаров</a>
...
<div class="box">
    <div class="box-content filterpro">
        <form id="filterpro">
            <div class="filterpro_title">Фильтр товара</div>
            ...
            <input type="hidden" name="limit" id="filterpro_limit" value="<?php echo $libusha; ?>">
            <input type="hidden" name="get_path" value="<?php echo $request_path; ?>">
            ...
            <div class="price_slider option_box" <?php if(!$price_slider) { echo 'style="display:none"';}?>>
                 <div class="option_name" style="background-image: none;">Цена:</div>

                <div class="ot_do clearfix">
                    <label>От<div class="input-border"><input type="text" class="price_limit" type="text" name="min_price" value="-1" id="min_price"></div></label>
                    <label>до<div class="input-border"><input type="text" class="price_limit" type="text" name="max_price" value="-1" id="max_price"></div></label>
                </div>
...
<div class="box">
    ...
    <div class="box-content filterpro">
        ...
        <form id="filterpro">
            ...
            <div class="option_box" <?php if(!$instock_visible) echo 'style="display:none"'; ?>>
                 <input type="checkbox" class="filtered" name="instock" id="instock" onchange="displayShowButton('instock', 'instockLable')"<?php if($instock_checked) echo 'checked="checked"'; ?>>
                   <label for="instock" id="instockLable"><?php echo $text_instock?></label>
            </div>
                        ...
                        <tr>
                            <td>
                                <input id="manufacturer_<?php echo $manufacturer['manufacturer_id']?>" class="manufacturer_value filtered"
                                       type="checkbox" name="manufacturer[]"
                                       value="<?php echo $manufacturer['manufacturer_id']?>"
                                       onchange="displayShowButton('<?php echo "manufacturer_".$manufacturer["manufacturer_id"]?>', '<?php echo "label".$manufacturer["manufacturer_id"]?>')">
                            </td>
                            <td>
                                <label for="manufacturer_<?php echo $manufacturer['manufacturer_id']?>"
                                       id = "label<?php echo $manufacturer['manufacturer_id']?>"><?php echo $manufacturer['name']?></label>
                            </td>
                        </tr>
                        <?php } ?>
                    </table>
                            ...
                            <tr>
                                <td>
                                    <input class="filtered a_name"
                                           ...
                                           value="<?php echo $value ?>" onchange="displayShowButton('attribute_value_<?php echo $attribute_value_id . $i; ?>', 'label_id_<?php echo $attribute_value_id . $i; ?>')">
                                </td>
                                <td>
                                    <label for="attribute_value_<?php echo $attribute_value_id . $i; ?>"
                                           ...
                                           id = "label_id_<?php echo $attribute_value_id . $i; ?>"
                                           class = "clearLable">
                                        <?php echo $value?>
                       ...
                       <tr>
                        <td>
                            <input class="filtered option_value" id="option_value_<?php echo $option_value['option_value_id']?>"
								   ...
								   onchange="displayShowButton('option_value_<?php echo $option_value['option_value_id']?>', '<?php echo "label_option_".$option_value['option_value_id']?>')"></td>
                        <td>
                            <label id="<?php echo "label_option_".$option_value['option_value_id'];?>" for="option_value_<?php echo $option_value['option_value_id']?>"><?php echo $option_value['name']?></label>
                        </td>
                    </tr>
                    <?php } ?>
                </table>
...
<script>
    var active_button=new Array();
    function displayShowButton(thisElementId, labelId) {
        var ElementId=parseInt(labelId.replace("label_id_", ""));
        $('.hideA').hide();
        element = "#" + thisElementId;
        labelId = "#" + labelId;
        if ($(element).is(':checked')) {
            active_button.push(ElementId); // add elements
            active_button= $.unique(active_button)
            $(labelId).after('<span onclick="iF();" class="primenitF hideA">Показать</span>');
        }
        if (!$(element).is(':checked')) {
            if (active_button.length > 0) {
			 labelId = "#label_id_" + lastClick(active_button,ElementId);
               $(labelId).after('<span onclick="iF();" class="primenitF hideA">Показать</span>');
            }
        }	 
    }

    function lastClick(active_button,ElementId){
		for(i=0; i<active_button.length; i++){
		if(typeof active_button[i] === 'undefined'){
		 }else{
			if(active_button[i]==ElementId){
				active_button.splice(i,1)
			}
		 };		
		}		
		return active_button[active_button.length-1]
    }
