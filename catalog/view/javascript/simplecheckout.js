...
function simplecheckout_submit() {
    var total= parseFloat($('#totals_shipping').val());
     var payment_method= parseInt($('#payment_method').val());
	 
    if (payment_method == 1) {
        // 
        $('.payment-info').remove();
        if (total < 1000 && $('.radio-cash_plusplus').is(':checked')){
            $('.title-cash_plusplus').append('<span class="payment-info">Для заказа нужно иметь товаров больше 1000р в корзине</span>');
            $('.radio-cash_plusplus').attr('disabled', 'disabled').prop('checked', false);
        }
		else {
		    $('.payment-info').hide();
			// my code
            ...
            //$('#simplecheckout_payment .simplecheckout-methods-table .code input').removeAttr('disabled').prop('checked', true);
    ...
    jQuery.ajax({
        ...
        success: function(data) {
            ...
            if(payment_method==1){
             ...
             if (total > 1000) {
                    $('.payment-info').hide();
                    //$('.radio-cash_plusplus').removeAttr('disabled').prop('checked', true); 
               
            } else if ($('.radio-cash_plusplus').is(':checked')){
                 $('.title-cash_plusplus').append('<span class="payment-info">Для заказа нужно иметь товаров больше 1000р в корзине</span>');
                $('.radio-cash_plusplus').attr('disabled', 'disabled').prop('checked', false);
                $('#simplecheckout_button_confirm').css({'opacity':'0.5'});
            }   
            }
...
jQuery(function() {
   ...
   var payment_method= parseInt($('#payment_method').val());
    ...
    /*if(total<1000 && payment_method==1){
        if(!$('.radio-cash_plusplus').is(':checked')){
        $('.title-cash_plusplus').append('<span class="payment-info">Для заказа нужно иметь товаров больше 1000р в корзине</span>');
        $('.radio-cash_plusplus').attr('disabled','disabled').prop('checked', false); 
        $('#simplecheckout_button_confirm').css({'opacity':'0.5'});
        }
    ...
    jQuery('.simplecheckout-cart-total-remove, .simplecheckout-cart-total-remove').live('click', function() {
        jQuery.ajax({
        url: 'index.php?' + simple_route + 'route=checkout/simplecheckout/clear_cart',        
        type: 'POST',
        dataType: 'text',
        success: function() {
            location.reload();
	...
	// oneclik-customer
    $('#oneclik-customer').live('click', function(e) {
        e.preventDefault();
        console.log("orderAll");
        var orderPhone = $('#oneclikPhone').val();
        var orderName = $('#oneclikName').val();
        var error = true;

        if (orderPhone.length < 3) {
            $('#oneclikPhone').css('border', '1px solid red');
            error = false;
        } else {
            error = true;
            $('#oneclikPhone').css('border', '1px solid #ccc');
        }

        if (orderName.length < 3) {
            $('#oneclikName').css('border', '1px solid red');
            error = false;
        } else {
            error = true;
            $('#oneclikName').css('border', '1px solid #ccc');
        }

        if (orderName.length > 3 && orderPhone.length > 3) {
            $.ajax({
                type: 'post',
	...
	$('.popup-container .x, #cboxOverlay').live('click',function(){
		location.reload();
	});
    // end oneclik-customer
