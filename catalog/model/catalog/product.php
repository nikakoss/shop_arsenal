...
class ModelCatalogProduct extends Model {
	...
	public function getProduct($product_id) {
		...
		// add by cmd
		if (!isset($query->row['stock_status']) || $query->row['stock_status'] === NULL) {
			$query->row['stock_status'] = 'Уточнять по телефону';
		}
	...
	public function getLatestProducts($limit) {
		...
		return $product_data;
        	...
        	public function getLatestIdProducts($id) {
		if ($this->customer->isLogged()) {
			$customer_group_id = $this->customer->getCustomerGroupId();
		} else {
			$customer_group_id = $this->config->get('config_customer_group_id');
		...
		$limit=1;		
		$product_data = $this->cache->get('product.latest.' . (int)$this->config->get('config_language_id') . '.' . (int)$this->config->get('config_store_id') . '.' . $customer_group_id . '.' . (int)$limit);
                var_dump((int)$this->config->get('config_language_id')); exit;
		if (!$product_data) { 
			$query = $this->db->query("SELECT p.product_id FROM " . DB_PREFIX . "product p LEFT JOIN " . DB_PREFIX . "product_to_store p2s ON (p.product_id = p2s.product_id) WHERE p.status = '1' AND p.date_available <= NOW() AND p2s.store_id = '" . (int)$this->config->get('config_store_id') . "' ORDER BY p.date_added DESC LIMIT 1" . (int)$limit);
	...
	public function getProductAttributes($product_id, $limit = false) {
	
		$product_attribute_group_data = array();
		
		$product_attribute_group_query = $this->db->query(
		"SELECT ag.attribute_group_id, agd.name FROM " . DB_PREFIX . "product_attribute pa 
		...
		ORDER BY ag.sort_order, agd.name ". ( $limit ? " LIMIT 1 " : "" )
		);
		...
		foreach ($product_attribute_group_query->rows as $product_attribute_group) {
			...
			"SELECT a.attribute_id, ad.name, pa.text FROM " . DB_PREFIX . "product_attribute pa 
				LEFT JOIN " . DB_PREFIX . "attribute a ON (pa.attribute_id = a.attribute_id) 
				LEFT JOIN " . DB_PREFIX . "attribute_description ad ON (a.attribute_id = ad.attribute_id) 
			WHERE 
				pa.product_id = '" . (int)$product_id . "' AND 
				a.attribute_group_id = '" . (int)$product_attribute_group['attribute_group_id'] . "' AND 
	...
	public function getProductRelated($product_id) {
        ...
        public function getProductCategoryRelated($product_id) {
        $query = $this->db->query("SELECT pr.related_id,cd.category_id,cd.name FROM " . DB_PREFIX . "product_related pr 
LEFT JOIN " . DB_PREFIX . "product p ON (pr.related_id = p.product_id) 
LEFT JOIN " . DB_PREFIX . "product_to_store p2s ON (p.product_id = p2s.product_id) 
LEFT JOIN " . DB_PREFIX . "product_to_category pc ON (pr.related_id = pc.product_id) 
LEFT JOIN " . DB_PREFIX . "category_description cd ON (pc.category_id = cd.category_id) 
WHERE pr.product_id = '".$product_id."' AND p.status = '1' AND p.date_available <= NOW() AND p2s.store_id = '0'");
        $CategoryProducts = $this->group_assoc($query->rows, 'category_id');
        $product_data=array();
        foreach ($CategoryProducts as $key=>$value){
            $product_array=array();
            foreach ($value as $item){
                $product_array[]=$this->getProduct($item['related_id']);
                $name=$item['name'];
            }
            $product_data[$key]['category_id']=$key;
            $product_data[$key]['name']=$name;
            $product_data[$key]['product_array']=$product_array;
        }
        return $product_data;
    }
