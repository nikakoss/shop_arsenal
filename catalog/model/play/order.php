...
class ModelPlayOrder extends Model {
    ...
    public function getStatus($user_email, $user_phone, $order_number) {
		if ($user_email != ''){
		$qry = $this->db->query("
            SELECT t2.name 
            FROM `" . DB_PREFIX . "order` AS t1, `" . DB_PREFIX . "order_status` AS t2 
            WHERE t1.email = '$user_email' AND t1.order_id = $order_number AND t1.order_status_id = t2.order_status_id");
		}
		else {
		$qry = $this->db->query("
            SELECT t2.name 
            FROM `" . DB_PREFIX . "order` AS t1, `" . DB_PREFIX . "order_status` AS t2 
            WHERE t1.telephone = '$user_phone' AND t1.order_id = $order_number AND t1.order_status_id = t2.order_status_id");
     	}
		return $qry;
