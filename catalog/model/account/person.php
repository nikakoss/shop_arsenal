    ...
    public function addCustomerAjaxModel($data) {
        if (isset($data['email']) && isset($data['password'])) {
            ...
            if ($emails) {
                ...
                if ($this->db->query("INSERT INTO " . DB_PREFIX . "customer SET person = 'f', email = '" . $this->db->escape($data['email']) . "', salt = '" . $this->db->escape($salt = substr(md5(uniqid(rand(), true)), 0, 9)) . "', password = '" . $this->db->escape(sha1($salt . sha1($salt . sha1($data['password'])))) . "', ip = '" . $this->db->escape($this->request->server['REMOTE_ADDR']) . "', status = '1', approved = '1'")) {
                    echo 'true';
                }
            }
        }
