...
class ControllerModuleFilterPro extends Controller {
	...
	protected function index($setting) {
		$this->config->set('ControllerModuleFilterPro', true);
		...
		if($setting['type'] == 1) {
			...
			$this->data['heading_title'] = $this->language->get('heading_title');
                        $this->data['request_path'] = $this->url->link('product/category', 'path=' . $this->request->get['path']);
			...
			foreach($this->data['attributes'] as $j => $attribute_group) {
				foreach($attribute_group['attribute_values'] as $attribute_id => $attribute) {
					...
					if($display_attribute != 'none') {
                        ...
                        if(isset($this->request->get['limit'])){
                            $this->data['libusha'] = $this->request->get['limit'];
                        } else {
                            $this->data['libusha'] = 10;    
                        }
            ...
            //---------------------my if-----------------------------------------
            if ($this->request->get['route'] == 'product/advanced') {
                if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/module/filterpro2.tpl')) {
                    $this->template = $this->config->get('config_template') . '/template/module/filterpro2.tpl';
                } else {
                    $this->template = 'default/template/module/filterpro2.tpl';
                }
            }
            //-------------------------------------------------------------------
            else {
	...
	public function getProducts() {
        ...
        $data = array(
			...
			'min_price' => isset($this->request->post['min_price']) ? $this->request->post['min_price'] : 0 / $this->k,
			'max_price' => isset($this->request->post['max_price']) ? $this->request->post['max_price'] : 999999999999 / $this->k,
			'start' => ($page - 1) * $limit,
			'limit' => $limit,
			'sort' => $sort,
			'order' => $order
		...
		$pagination->url = $this->request->post['get_path'].'&page={page}';
	...
	private function getHtmlProducts($results, $product_total) {
		...
		foreach ($results as $result) {
			...
			$this->data['text_on'] = $this->language->get('text_on');
								...
								$product_status = $this->model_catalog_product->getProduct($result['product_id']);
				...
				'special'     => $special,
				'tax'         => $tax,
				'attributes'  => $attribute_groups, // add
				'rating'      => $result['rating'],
				'reviews'     => sprintf($this->language->get('text_reviews'), (int)$result['reviews']),
                'total' =>   (int) $review_total,
                'rait_s' =>   $rait_s_total,
				'status' => $product_status['stock_status'],
