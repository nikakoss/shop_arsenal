...
class ControllerPaymentPayanywayMM extends Controller {
	protected function index() {
		...
		$this->data['paymentSystemUnitId'] = 1038;
		...
		$this->data['entry_moneymail_email'] = $this->language->get('entry_moneymail_email');
		$this->data['entry_moneyMailError'] = $this->language->get('entry_moneyMailError');

		if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/payment/payanyway_mm.tpl')) {
			$this->template = $this->config->get('config_template') . '/template/payment/payanyway_mm.tpl';
		} else {
			$this->template = 'default/template/payment/payanyway_mm.tpl';
		}	
