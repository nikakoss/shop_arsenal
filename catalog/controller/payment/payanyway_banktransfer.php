...
class ControllerPaymentPayanywayBanktransfer extends Controller {
	protected function index() {
		...
		$this->data['paymentSystemUnitId'] = 705000;
		$this->data['paymentSystemAccountId'] = 75983431;
		...
		if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/payment/payanyway_banktransfer.tpl')) {
			$this->template = $this->config->get('config_template') . '/template/payment/payanyway_banktransfer.tpl';
		} else {
			$this->template = 'default/template/payment/payanyway_banktransfer.tpl';
		}
