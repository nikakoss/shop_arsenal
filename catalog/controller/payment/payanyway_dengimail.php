...
class ControllerPaymentPayanywayDengimail extends Controller {
	protected function index() {
		...
		$this->data['paymentSystemUnitId'] = 545234;
		...
		$this->data['entry_dengimail_email'] = $this->language->get('entry_dengimail_email');
		$this->data['entry_dengimail_error'] = $this->language->get('entry_dengimail_error');

		if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/payment/payanyway_dengimail.tpl')) {
			$this->template = $this->config->get('config_template') . '/template/payment/payanyway_dengimail.tpl';
		} else {
			$this->template = 'default/template/payment/payanyway_dengimail.tpl';
		}	
