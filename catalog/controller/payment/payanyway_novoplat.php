...
class ControllerPaymentPayanywayNovoplat extends Controller {
	protected function index() {
		...
		$this->data['paymentSystemUnitId'] = 281129;
		$this->data['paymentSystemAccountId'] = 80314912;
		...
		$this->data['entry_netellerAccount'] = $this->language->get('entry_netellerAccount');
		$this->data['entry_netellerSecureId'] = $this->language->get('entry_netellerSecureId');
		$this->data['entry_netellerBankAcct'] = $this->language->get('entry_netellerBankAcct');

		if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/payment/payanyway_novoplat.tpl')) {
			$this->template = $this->config->get('config_template') . '/template/payment/payanyway_novoplat.tpl';
		} else {
			$this->template = 'default/template/payment/payanyway_novoplat.tpl';
		}	
