...
class ControllerPaymentPayanywayComepay extends Controller {
	protected function index() {
		...
		$this->data['paymentSystemUnitId'] = 228820;
		$this->data['paymentSystemAccountId'] = 47654606;
		...
		if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/payment/payanyway_comepay.tpl')) {
			$this->template = $this->config->get('config_template') . '/template/payment/payanyway_comepay.tpl';
		} else {
			$this->template = 'default/template/payment/payanyway_comepay.tpl';
		}
