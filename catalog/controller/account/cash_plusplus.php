        ...
        if (isset($_GET['sum']) & isset($_GET['onum']) & isset($_GET['plat'])) {
            ...
            if (isset($address['firstname']) & isset($address['lastname'])) {
                ...
                if ($_GET['plat'] == $platp & ($_GET['sum'] == (intval(str_replace(" ","", $address['total']) + 1)) || $_GET['sum'] == intval(str_replace(" ","", $address['total']))) & $_GET['onum'] === $address['order_id']) {
                    $this->session->data['this_payment_methods'] = $address['payment_method'];
                    ...
                    if ($gruzpay == 'gruzpay_custom'){
                      ...
                      $customz = "";
                        ...
                        if ($custom == 'kpp_y'){
                         ...
                         if ($custom == 'organization_y'){
                          $custom_other = 'Название организации: ';
                        }
                        if ($custom == 'cal_account_y'){
                          $custom_other = 'Расчетный счет: ';
                        }
                        if ($custom == 'bank_detail_y'){
                          $custom_other = 'Банковские реквизиты: ';
                        }
                        if ($custom == 'address_y_y'){
                          $custom_other = 'Юридический адрес: ';
                        }
                        if ($custom == 'address_f_y'){
                          $custom_other = 'Фактический адрес: ';
                        }
                        if ($custom == 'contact_person_y'){
                          $custom_other = 'Контактное лицо: ';
                        }
                        else{
                          if (isset($address[$custom])) {
                        $custom_other = $address[$custom];
