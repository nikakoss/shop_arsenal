...
class ControllerCheckoutSimpleCheckoutCart extends Controller {
    ...
    public function index() {
        ...
        if ($this->customer->isLogged()) {
            ...
            $TotalOrders = $this->model_checkout_coupon->getStatusAcount();
        }

        if ($this->customer->isLogged()) {
            $this->load->model('checkout/coupon');
            $getStatusAcount = $this->model_checkout_coupon->getStatusAcount();
            $default_discount = $getStatusAcount['discount'];
        }
        
        $total_default_with_discount = 0;
        foreach ($products as $product) {
            ...
            if ($product['image']) {
                ...
                $image_cart_width = $image_cart_width ? $image_cart_width : 88;
                $image_cart_height = $this->config->get('config_image_cart_height');
                $image_cart_height = $image_cart_height ? $image_cart_height : 88;
                $image = $this->model_tool_image->resize($product['image'], $image_cart_width, $image_cart_height);
            } else {
                $image = $this->model_tool_image->resize('data/no_image.jpg', $this->config->get('config_image_cart_width'), $this->config->get('config_image_cart_height'));
            }
            ...
            if ($this->customer->isLogged()) {
				...
				for($i=count($coupon)-1;$i >= 0;$i--){

                if ($coupon && $getStatusAcount['id']==$coupon[$i]['status_client']) {
					if($product_info['price'] != $product['price']){
						$product['price'] = $product_info['price'];
					}
                    if ($coupon[$i]['type'] == 'P') {
                            $product['price'] = $product['price'] - (($product['price'] / 100) * $coupon[$i]['discount']);
							
                            $discount_price = round($coupon[$i]['discount'],1) . "%";          
						$total=$product['price']*(int)$product['quantity'];
                    }

                    if ($coupon[$i]['type'] == 'F') {
                            $product['price'] = $product['price'] - $coupon[$i]['discount'];
                            $discount_price = round($coupon[$i]['discount'],2) . " руб";   
						$total=$product['price']*(int)$product['quantity'];	                
                    }		
					
				//	$total = str_replace(",", "", $total);
                   // $total = str_replace("руб", "", $total);
				//	var_dump($product['price']); exit;
				 ...
				 // formating price
			  if (($this->config->get('config_customer_price') && $this->customer->isLogged()) || !$this->config->get('config_customer_price')) {
                    $total = $this->currency->format($this->tax->calculate($total, $this->config->get('config_tax')));
                } else {
                    $total = false;
                }
            ...
            $total_default_with_discount += $total2;
        ...
        $this->session->data['total_default_with_discount'] = $total_default_with_discount;
        // Gift Voucher
